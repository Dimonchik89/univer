...
export class PushService {
  ...
  private async sendNotification({
    ...
    const sendPromises = subscriptions.map((sub) => {
      ...
      return webpush
        .sendNotification(tailSub, payload) // вiдправляэмо повыдомлення
        .catch(async (error) => {
          this.logger.error(
            `Error sending push: ${error.statusCode} for ID ${sub.endpoint}`,
          );
  ...
  async sendNewMessageNotification({
    ...
    await this.sendNotification({
      subscriptions: subscriptionFinally,
      title: messageTitle,
      scheduledAt,
    });
  }
