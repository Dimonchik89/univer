...
export class PushController {
  ...
  @ApiOperation({
    summary: swaggerPushSubscription.CREATE_SUBSCRIPTION_SUMMARY,
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiOperation({
    summary: swaggerPushSubscription.DELETE_SUBSCRIPTION_SUMMARY,
  })
  @ApiResponse({
    status: 200,
  ...
  async unsubscribe(@Req() req, @Body() dto: DeleteSubscriptionDto) {
    ...
    await this.pushService.deleteSubscription({
      userId: req.user.id,
