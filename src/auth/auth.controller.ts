...
import { FORGOT_PASSWORD_MESSAGE, INVALID_REFRESH_TOKEN, PASSWORD_RESET_SUCCESS, SIGNOUT_SUCCESS_MESSAGE } from './constants/auth.constants';
...
export class AuthController {
    ...
    async register(@Body() createAuthDto: CreateAuthDto, @Res({ passthrough: true }) res: Response) {
		...
		const { access_token, refresh_token } = await this.authService.register(createAuthDto);
    ...
    async refresh(@Req() req, @Res({ passthrough: true }) res: Response) {
        ...
        const { access_token, refresh_token } = await this.authService.refresh(req.user?.id, req.user?.roles);
