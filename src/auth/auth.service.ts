...
export class AuthService implements OnModuleInit {
	...
	async login({ id, roles }: { id: string, roles: Role[]}) {
		const { access_token, refresh_token } = await this.generateTokens({ id, roles });
	...
	async generateTokens(
		...
		this.jwtService.signAsync<AuthJwtPayload>({ id: user.id, roles: user.roles }),
		this.jwtService.signAsync<{ id: string }>({ id: user.id }, this.refreshTokenConfig),
		]);
	...
	async validateRefreshToken({ userId, refreshToken }: { userId: string, refreshToken: string }) {
		const user = await this.userRepository.findOne({
			where: {
				id: userId
			},
	...
	addTokensToResponseCookies({ access_token, refresh_token, res }: { access_token: string, refresh_token: string, res: Response }) {
