...
//   }
	...
	@ApiOperation({ summary: "Get all users (You need to add an administrator access_token or other users who have the required permissions)"})
	...
	async findOne(@Param("id") id: string, @Request() req: any) {
		return await this.userService.findOne({ userIdFromToken: req.user.id, userIdFromParam: id });
	}

	@ApiOperation({ summary: "Get user profile. Endpoint for admins (You need to add an administrator access_token or other users who have the required permissions)" })
	@ApiHeader(swaggerConstants.HEADER_ACCESS_TOKEN_EXAMPLE)
	@ApiResponse({
		status: 200,
		description: swaggerConstants.SUCCESSFUL_MESSAGE,
	...
	@ApiOperation({ summary: "Change user profile by admin (You need to add an administrator access_token or other users who have the required permissions)" })
	@ApiBody({ type: UpdateUserDto })
	@ApiHeader(swaggerConstants.HEADER_ACCESS_TOKEN_EXAMPLE)
	@ApiResponse({
		status: 200,
